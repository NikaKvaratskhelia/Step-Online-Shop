...
export class FavoritesComponent implements OnInit, OnDestroy {
  ...
  getProduct(id: string) {
    this.tools.getProductId(id).pipe(
      takeUntil(this.destroy$),
      tap((data) => {
  ...
  removeFromFavs(productId: string) {
    ...
    this.tools.addToFavs(this.id, this.favProductIDs).pipe(
      takeUntil(this.destroy$),
      tap(() => {
        this.favProducts = this.favProducts.filter(product => product._id !== productId);
